...
const statCards = [
    ...
    { title: "Partner Institutions", value: "45", icon: Activity, trend: "+5", sub: "Academic, govt & private sector" },
    { title: "Graduation Rate", value: "98.2%", icon: TrendingUp, trend: "+1.1%", sub: "Industry-leading outcome" },
    { title: "Sector Placements", value: "850+", icon: BarChart3, trend: "+24%", sub: "Within 6 months of graduation" },
    { title: "States Represented", value: "36 + FCT", icon: MapPin, trend: "100%", sub: "Full national representation" },
    { title: "Funds Deployed", value: "₦18.6B", icon: PieChart, trend: "↑ YoY", sub: "100% to scholar tuition & stipends" },
];
        ...
        <div className="flex flex-col w-full">
                    ...
                    <Card className="border-border/50">
                        <CardContent className="p-6">
                            ...
                            <div className="space-y-4">
                                {[
                                    { label: "Active Scholars", value: "890", color: "bg-primary" },
                                    { label: "Graduated", value: "360", color: "bg-emerald-500" },
                                    { label: "Fully Employed", value: "850+", color: "bg-blue-500" },
                                    { label: "In Postgrad Study", value: "42", color: "bg-purple-500" },
                                ].map((row, i) => (
            ...
            <SectionWrapper title="Scholar Outcomes" description="Tracking real impact — from first intake to national deployment." className="bg-muted/20 border-t">
                <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5 mt-8">
                    {mockScholars.slice(0, 4).map((scholar) => (
                            ...
                            <CardContent className="p-5">
                                <div className="flex items-center gap-3 mb-4">
                                    ...
                                    <div className="min-w-0">
                                        <p className="font-semibold text-sm truncate">{scholar.name}</p>
                                        <Badge variant="outline" className="text-xs capitalize mt-0.5">{scholar.status}</Badge>
                                    </div>
                                </div>
                                <div className="space-y-1.5 text-xs text-muted-foreground">
                                    <div className="flex items-center gap-1.5"><GraduationCap className="h-3.5 w-3.5 shrink-0" />{scholar.discipline}</div>
                                    <div className="flex items-center gap-1.5"><MapPin className="h-3.5 w-3.5 shrink-0" />{scholar.state} State</div>
                                    <div className="flex items-start gap-1.5 mt-2 pt-2 border-t">
                                        <Activity className="h-3.5 w-3.5 shrink-0 mt-0.5" />
                                        <span className="font-medium text-foreground">{scholar.placement}</span>
                                    </div>
                                </div>
                            </CardContent>
                        </Card>
